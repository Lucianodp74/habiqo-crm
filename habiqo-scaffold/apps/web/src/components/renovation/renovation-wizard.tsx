'use client'
// components/renovation/renovation-wizard.tsx
// Destinazione: apps/web/src/components/renovation/renovation-wizard.tsx
// ─────────────────────────────────────────────────────────────────
...
export function RenovationWizard({ propertyId }: Props) {
  ...
  // ── Timer elapsed durante generazione ─────────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      // Timeout dopo ~6 minuti (120 poll × 3 secondi)
      ...
      try {
        ...
        if (!res.ok) return // Network error — riprova al prossimo tick
        ...
        // Errore di rete temporaneo — continua polling
      }
    }, 3_000)
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const uploadRes = await fetch('/api/renovation/upload', {
        method: 'POST',
        body:   formData,
      })
      ...
      const { previewId: id, beforeImageUrl: bUrl } =
        (await uploadRes.json()) as UploadResponse

      setPreviewId(id)
      setBeforeImageUrl(bUrl)

      // 2. Avvia generazione AI
      ...
      // 3. Avvia polling (gestito da useEffect)
      setStep('generating')
    ...
    <div className="w-full max-w-2xl mx-auto">
      ...
      {step === 'configure' && (
        <div className="space-y-6">
          ...
          {/* Upload zone o preview immagine */}
          {!file ? (
            <div
              role="button"
              tabIndex={0}
            ...
            >
              <svg
                className="w-10 h-10 text-stone-300 mb-3"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                aria-hidden="true"
              ...
              <p className="text-xs text-stone-400 mt-1">
                o clicca per selezionare · JPG, PNG, WEBP · max 10 MB
              </p>
              <input
                ref={inputRef}
                type="file"
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {STYLES.map((s) => (
                ...
                >
                  {style === s.value && (
                    <div className="w-2 h-2 rounded-full bg-stone-900 mb-3" />
                  )}
          ...
          <button
            ...
            className={[
              'w-full py-4 rounded-xl text-sm font-medium tracking-wide',
        ...
        <div className="text-center py-10 space-y-6">
          ...
          <div>
            <p className="font-medium text-stone-900">
              {selectedStyle?.label ?? 'Stile selezionato'} · Stima ~45 sec
            </p>
            <p className="text-sm text-stone-400 mt-1">
              Elaborazione in corso · {elapsedStr}
            </p>
          </div>
      ...
      {step === 'error' && (
        <div className="text-center py-12 space-y-4">
          ...
          <div className="flex items-center gap-3 justify-center pt-2">
            {/* Riprova: mantiene il file e i settings, riavvia da generate */}
            <button
              onClick={() => {
                setStep('configure')
                setError(null)
