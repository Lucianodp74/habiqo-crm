 ...
 *
 * Feed XML per Idealista Italy (+ Casa.it).
 * Include solo proprietà dove published_to contiene "idealista".
...
export async function GET(_req: Request, { params }: { params: Params }) {
  ...
  // Filtra: published_to contiene "idealista" OPPURE è vuoto (backward compat)
  const filtered = (propertiesData ?? []).filter((p) => {
    const portals: string[] = p.published_to ?? [];
    return portals.length === 0 || portals.includes("idealista");
  });
